...
def get_max_memory(usage_threshold_percent: float = 10):
    pynvml.nvmlInit()

    visible_devices_str = os.environ.get("CUDA_VISIBLE_DEVICES")
    if visible_devices_str:
        visible_ids = [int(x.strip()) for x in visible_devices_str.split(",")]
    else:
    ...
    for idx in visible_ids:
        ...
        # print(f"GPU {idx}: {total_mb:.1f}MB total, {used_mb:.1f}MB used ({used_percent:.1f}%)")

        if used_percent > usage_threshold_percent:
            print(f"DEVICE {idx} is using {used_percent:.1f}% of its memory. It will not be used for embedding model.")
            max_memory[idx] = "0GB"
        else:
            free_mb = total_mb - used_mb
            free_gb = free_mb / 1024
            print(
                f"DEVICE {idx} is using {used_percent:.1f}% of its memory, has {free_gb:.1f}GB free. It is available for embedding model."
            )
            max_memory[idx] = f"{free_gb:.1f}GB"
    reorganized_max_memory = {}
    for i, idx in enumerate(visible_ids):
        reorganized_max_memory[i] = max_memory[idx]
    return reorganized_max_memory
