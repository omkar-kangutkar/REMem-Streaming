...
class PromptTemplateManager:
    ...
    def __post_init__(self) -> None:
        ...
        # abs path to dir where each *.py file (exclude __init__.py) contains a variable prompt_tempalte (a str or a chat hisotry with content as raw str for being converted to a Template)
        self.templates_dir = os.path.join(package_dir, "templates")
    ...
    def _load_templates(self) -> None:
        ...
        logger.debug(f"Loading templates from directory: {self.templates_dir}")
        for filename in os.listdir(self.templates_dir):
            if filename.endswith(".py") and filename != "__init__.py":
                script_name = os.path.splitext(filename)[0]
                module_name = f"remem.prompts.templates.{script_name}"
