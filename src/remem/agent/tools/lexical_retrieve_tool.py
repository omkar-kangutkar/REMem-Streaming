...
class LexicalRetrieveTool(BaseTool):
    """Tool for lexical retrieval using BM25 that retrieves both gists and triples simultaneously."""

    def __init__(self, remem_instance):
        super().__init__(ToolType.LEXICAL_RETRIEVE, remem_instance)
        self._bm25_retrievers = {}  # Cache BM25 retrievers by entry type
        self._formatted_fact_cache = {}  # Cache formatted fact content
        ...
        return json.dumps(
            {
                ...
                "description": "Search for the most relevant 'gists' and 'facts' (triples) in the KG based on BM25 scoring. It returns the top results from both types at once. Use when you need keyword-based or exact term matching, e.g., identifiers.",
                "parameters": {
                    "type": "object",
                    "properties": {
    ...
    def execute(self, context: ToolContext, beam_size: int = 10, do_filter: bool = False, **kwargs) -> ToolResult:
        """Execute lexical retrieval using BM25 based on temporal context."""
        try:
            # Check if available_entries is empty or None
            if not context.available_entries:
                self.logger.error("LexicalRetrieveTool: available_entries is None or empty - no corpus to search")
                return ToolResult(
                    tool_type=self.tool_type,
                    nodes_found=[],
                    scores=[],
                    observation="No available entries to search",
                    should_continue=False,
                    node_contents=[],
                )

            # Log available entry types and counts
            for entry_type, entries in context.available_entries.items():
                count = len(entries) if entries else 0
                if count == 0:
                    self.logger.error(f"LexicalRetrieveTool: {entry_type} corpus is empty")
                # else:
                #     self.logger.info(f"LexicalRetrieveTool: {entry_type} corpus has {count} entries")
            ...
            temporal_context = kwargs.get("temporal_context", True)
            ...
            if not temporal_context:
                ...
                if "verbatim" in context.available_entries:
                    verbatim_scores = self._get_entry_bm25_scores(
                        context.query, "verbatim", context.available_entries["verbatim"], beam_size
                    )
                    verbatim_keys = context.available_entries["verbatim"]

                    verbatim_candidates = []
                    for i, (key, score) in enumerate(zip(verbatim_keys, verbatim_scores)):
                        if not self._is_visited(key, context):
                            verbatim_candidates.append((key, float(score)))
                ...
                # Retrieve gists using BM25
                if "gists" in context.available_entries:
                    gist_scores = self._get_entry_bm25_scores(
                        context.query, "gists", context.available_entries["gists"], beam_size
                    )
                    ...
                    for i, (key, score) in enumerate(zip(gist_keys, gist_scores)):
                        if not self._is_visited(key, context):
                            gist_candidates.append((key, float(score)))
                ...
                # Retrieve triples using BM25
                if "facts" in context.available_entries:
                    fact_scores = self._get_entry_bm25_scores(
                        context.query, "facts", context.available_entries["facts"], beam_size
                    )
                    ...
                    for i, (key, score) in enumerate(zip(fact_keys, fact_scores)):
                        ...
                        if not self._is_visited(key, context) and self._matches_temporal_filter(
                            ...
                            fact_candidates.append((key, float(score)))
            ...
            if temporal_context:
                ...
                display_gists = len(top_gists)
                display_triples = len(top_facts)
                ...
                if display_gists > 0 or display_triples > 0:
                    found_parts = []
                    if total_gists > 0:
                    ...
                    show_parts = []
                    if display_gists > 0:
                        show_parts.append(f"{display_gists} gists")
                    if display_triples > 0:
                        show_parts.append(f"{display_triples} triples")
                    observation = f"Retrieved {' and '.join(found_parts)} via BM25 lexical matching, showing {' and '.join(show_parts)}"
                else:
                    observation = "No relevant content found via BM25 lexical matching"
            else:
                total_verbatim = len(verbatim_candidates)
                display_verbatim = len(top_verbatim)
                total_verbatim_str = ">1000" if total_verbatim == 1000 else str(total_verbatim)

                if display_verbatim > 0:
                    observation = f"Retrieved {total_verbatim_str} verbatim texts via BM25 lexical matching, showing {display_verbatim} verbatim texts"
                else:
                    observation = "No relevant verbatim content found via BM25 lexical matching"
            ...
            self.logger.error(f"Error in lexical retrieve: {e}")
            return ToolResult(
                tool_type=self.tool_type,
                nodes_found=[],
                scores=[],
                observation=f"Error in lexical retrieve: {e}",
                should_continue=False,
                node_contents=[],
            )

    def _get_entry_bm25_scores(self, query: str, entry_type: str, entry_keys: List[str], top_k: int) -> List[float]:
        """Get BM25 scores between query and entries of given type."""
        try:
            # Create or get cached BM25 retriever for this entry type
            if entry_type not in self._bm25_retrievers:
                self._bm25_retrievers[entry_type] = self._build_bm25_retriever(entry_type, entry_keys)

            retriever = self._bm25_retrievers[entry_type]
            if retriever is None:
                return [0.0] * len(entry_keys)

            if top_k is None:
                top_k = len(entry_keys)
            # Get top-k results with scores (BM25SparseRetriever doesn't support scores_on_corpus)
            # We'll get all results and then map back to original order
            indices, scores = retriever.get_top_k_indices(query, k=top_k, return_scores=True)

            # Create a score array matching the original entry_keys order
            score_map = dict(zip(indices, scores))
            ordered_scores = []
            for i in range(len(entry_keys)):
                ordered_scores.append(score_map.get(i, 0.0))

            return ordered_scores

        except Exception as e:
            self.logger.warning(f"Error getting BM25 scores for {entry_type}: {e}")
            return [0.0] * len(entry_keys)

    def _build_bm25_retriever(self, entry_type: str, entry_keys: List[str]):
        """Build BM25 retriever for given entry type."""
        try:
            # Extract text content from all entries
            corpus = []
            for key in entry_keys:
                content = self._get_node_content_for_bm25(key, entry_type)
                corpus.append(content)

            if not corpus:
                return None

            # Create BM25 sparse retriever (using bm25s package)
            from remem.utils.retrieval_api import BM25SparseRetriever

            # Pass working_dir from ReMem instance for proper cache management
            working_dir = getattr(self.remem, "working_dir", None) if self.remem else None
            ...
            self.logger.error(f"Error building BM25 retriever for {entry_type}: {e}")
            return None

    def _get_node_content_for_bm25(self, node_key: str, entry_type: str) -> str:
        """Get content for a node formatted for BM25 indexing."""
        try:
            if self.remem and hasattr(self.remem, "episodic_embedding_stores"):
                store = self.remem.episodic_embedding_stores.get(entry_type)
                if store:
                    row = store.get_row(node_key)
                    if row and "content" in row:
                        content = row["content"]
                        if isinstance(content, dict):
                            # For facts/triples, use the retrieval format which includes temporal info
                            if entry_type == "facts" and all(k in content for k in ["subject", "predicate", "object"]):
                                return self._get_formatted_fact_content_for_retrieval(node_key)
                            # For gists, use text content
                            return content.get("text", str(content))
                        return str(content)
            return node_key
        except Exception as e:
            self.logger.warning(f"Error getting BM25 content for {node_key}: {e}")
            return node_key
