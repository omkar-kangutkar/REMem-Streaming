            ...
            "properties": {
                "triples": {
                    ...
                    "items": {
                        "type": "array",
                        "items": {"type": "string"},
                        # "minItems": 3, # not supported by OpoenAI structured outputs for now: https://platform.openai.com/docs/guides/structured-outputs?api-mode=chat
                        # "maxItems": 3
                    },
                }
            },
...
def parse_json_from_json_mode(json_string: str) -> dict:
    """
    Parses a string that loosely represents a JSON object, which may contain
    single-line comments (//) and trailing commas.

    This is useful for parsing JSON-like configurations that are more human-readable.
    ...
    """
    # Step 1: Remove single-line C-style comments (//...)
    # This regex finds '//' and matches all characters until the end of the line.
    comment_pattern = r"//.*"
    no_comments_string = re.sub(comment_pattern, "", json_string, flags=re.MULTILINE)

    # Step 2: Remove trailing commas
    # This regex finds commas that are followed by optional whitespace and then a
    # closing brace '}' or bracket ']'. It removes the comma but keeps the closing
    # character, thus fixing the invalid syntax.
    trailing_comma_pattern = r",\s*([}\]])"
    cleaned_string = re.sub(trailing_comma_pattern, r"\1", no_comments_string)

    try:
        # Step 3: Parse the cleaned string using the standard JSON library
        json_object = json.loads(cleaned_string)
        return json_object
    except json.JSONDecodeError as e:
        print(f"Error decoding JSON: {e}")
        # Printing the string that failed to parse is helpful for debugging
        print("Final string passed to the parser:")
