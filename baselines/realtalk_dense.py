...
from examples.realtalk import get_candidate_messages_realtalk, get_gold_docs_for_qa_pair, map_question_category
...
if __name__ == "__main__":
    ...
    # Per-question category aggregation (dataset field 'category')
    ...
    for sample_idx, file_path in enumerate(files):
        ...
        conversation = sample  # REALTALK root contains sessions
        qa_pairs = sample.get("qa", [])
        total_qa_pairs += len(qa_pairs)
        candidate_docs = get_candidate_messages_realtalk(conversation)

        print(f"sample {sample_idx} ({os.path.basename(file_path)}) # chunk contents:", len(candidate_docs))
        ...
        # Per-session per-question category aggregation (dataset field 'category')
        session_category_metrics = defaultdict(lambda: defaultdict(float))  # category -> metric -> value
        ...
        for qa_idx, qa_pair in enumerate(qa_pairs):
            ...
            # Per-question category from dataset field 'category'
            q_category = map_question_category(qa_pair.get("category"))
            ...
            if args.gold_retrieval:
                ...
                for gold_doc in gold_docs:
                    ...
                    content = gold_doc["messages"][0]["content"].split(":", 1)[1].strip()
                    role = gold_doc["messages"][0]["content"].split(":", 1)[0].strip()
                ...
                sorted_chunk_scores = np.array([1.0] * len(retrieved_chunk_contents))
            else:
            ...
            for key in retrieval_results:
                ...
                if q_category:
                    category_metrics[q_category][key] += retrieval_results[key]
                    session_category_metrics[q_category][key] += retrieval_results[key]
            for key in qa_results:
                total_metrics[key] += qa_results[key]
                session_metrics[key] += qa_results[key]
                if q_category:
                    category_metrics[q_category][key] += qa_results[key]
                    session_category_metrics[q_category][key] += qa_results[key]

            if q_category:
                category_counts[q_category] += 1
                session_category_counts[q_category] += 1

            # print current metrics
            for key in retrieval_results:
                print(key, round(total_metrics[key] / num_total_questions, 4), end="\t")
            print()
            for key in qa_results:
                print(key, round(total_metrics[key] / num_total_questions, 4), end="\t")
            print()
        ...
        all_samples.extend(samples_dict)

        # Add per-question category (ability) metrics if they exist for this session
        if session_category_counts:
            ...
            # Save under 'categories' (per-question dataset category)
    ...
    overall_metrics = {key: round(total_metrics[key] / max(1, num_total_questions), 4) for key in total_metrics}
    overall_results = {
        "total_qa_pairs": num_total_questions,
        "overall_metrics": overall_metrics,
        "samples": all_samples,
    }

    # Add category breakdown (temporal)
    if category_counts:
        # Overall per-question dataset category breakdown
        category_summary = {}
    ...
    overall_base_dir = "outputs/realtalk/realtalk_dpr_only"
    retrieval_label = ""
    if args.gold_retrieval:
        retrieval_label = "_gold"

    overall_results_file = f"{overall_base_dir}/overall_results_{llm_label}_{embedding_label}{retrieval_label}.json"
    os.makedirs(os.path.dirname(overall_results_file), exist_ok=True)
    safe_dump_json(overall_results, overall_results_file)

    print("Total QA pairs:", num_total_questions)
    print("\n=== Final Results ===")
