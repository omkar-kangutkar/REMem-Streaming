...
def extract_supporting_paragraphs(sample):
    supporting_paragraphs = []

    if "paragraphs" in sample:
        for paragraph in sample["paragraphs"]:
            if paragraph.get("is_supporting", False):
                supporting_paragraphs.append(paragraph["title"] + "\n" + paragraph["paragraph_text"])
...
def process_single_sample(sample_data):
    ...
    """
    sample_idx, sample, rag, config, args, selected_metrics, force_rag_from_scratch = sample_data
    ...
    rag_results_path = os.path.join(
        rag.working_dir + "/samples", f"rag_results_{inference_type}_sample_{sample_idx}.json"
    )
    ...
    try:
        ...
        # Extract gold documents (supporting paragraphs)
        ...
        all_gold_docs = [gold_paragraphs]  # Gold documents for this question
...
def main():
    ...
    parser.add_argument("--batch_size", type=int, default=16, help="Batch size for embedding model")
    ...
    # Load the dataset
    if args.debug:
        dataset_path = "reproduce/dataset/sample.json"
        corpus_path = "reproduce/dataset/sample_corpus.json"
    else:
    ...
    if args.llm_infer_mode == "offline":
        ...
        extract_llm_client = CacheOpenAI(
            f"outputs/{args.dataset}/extract_llm_cache", llm_name=extract_llm_label, llm_base_url=llm_base_url
        )
        config.extract_llm_label = extract_llm_label

        qa_llm_label = args.llm_name.replace("/", "_")
        qa_llm_client = CacheOpenAI(
            f"outputs/{args.dataset}/qa_llm_cache", llm_name=args.llm_name, llm_base_url=llm_base_url
        )
    ...
    working_dir = get_working_dir(config.save_dir, args.dataset, llm_label, embedding_label, args.extract_method)
    ...
    print(f"Indexing {len(corpus_texts)} corpus facts...")
    rag.index(corpus_texts)

    # Metrics for evaluation
    selected_metrics = ("qa_em", "qa_f1", "qa_bleu1", "qa_mem0_llm_judge")
    ...
    if args.parallel:
        ...
        sample_data_list = [
            (sample_idx, sample, rag, config, args, selected_metrics, force_rag_from_scratch)
        ...
        with ThreadPoolExecutor(max_workers=args.num_workers) as executor:
            ...
            pbar = tqdm(
                ...
                desc=f"Processing {args.dataset} samples (parallel)",
                unit="sample",
                leave=True,
            )
        ...
        failed_samples = []
        pbar = tqdm(
            samples_to_process, desc=f"Processing {args.dataset} samples (sequential)", unit="sample", leave=True
        )

        for sample_idx, sample in pbar:
            sample_data = (sample_idx, sample, rag, config, args, selected_metrics, force_rag_from_scratch)
    ...
    output_dir = f"outputs/{args.dataset}/{args.dataset}_{llm_label}_{embedding_label}"
