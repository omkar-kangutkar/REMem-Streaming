...
class StreamingIndexer:
    ...
    def __init__(self, remem_instance, log_latency: bool = True):
        ...
        # STREAMING FIX:
        # Keep the complete session history seen so far.
        self._session_history = []

    def start(self) -> None:
        import os

        # Clear the shared OpenIE results only once at the start of a fresh
        # streaming conversation. Do not clear it before every session.
    ...
    def stop(self) -> None:
        ...
        self._running = False
        logger.info(
            "StreamingIndexer stopped. Indexed: %d, Failed: %d",
            self.num_indexed,
            self.num_failed,
        )
    ...
    def _index_one_message(self, msg: dict) -> None:
        # Streaming REALTALK is indexed session-by-session.
        self.remem.global_config.preprocess_chunk_func = "by_session"

        # Build the first session from scratch. Every later session must
        # incrementally extend the existing graph, embeddings, and OpenIE state.
        if self.num_indexed == 0:
            self.remem.global_config.force_index_from_scratch = True
            self.remem.global_config.force_openie_from_scratch = True
        else:
            self.remem.global_config.force_index_from_scratch = False
            self.remem.global_config.force_openie_from_scratch = False
        ...
        # Accumulate all sessions seen so far.
        self._session_history.append(session)

        logger.info(
            "Streaming history now contains %d session(s).",
            len(self._session_history),
        )

        # REMem receives the full conversation history accumulated so far.
        self.remem.index(self._session_history)
